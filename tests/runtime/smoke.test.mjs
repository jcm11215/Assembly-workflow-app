...
const probes = [
  ...
  ['blueprints/ui: reviewPanelHtml', async()=>{ const m=await import('../../src/blueprints/ui.js'); m.reviewPanelHtml(state.jobs[0]); }],
  ['blueprints/ui: engineeringPanelHtml', async()=>{ const m=await import('../../src/blueprints/ui.js'); m.engineeringPanelHtml(state.jobs[0]); }],
  ['blueprints/ui: verificationReportHtml', async()=>{ const m=await import('../../src/blueprints/ui.js'); m.verificationReportHtml(state.jobs[0]); }],
  ['blueprints/ui: openVersionHistory', async()=>{ const m=await import('../../src/blueprints/ui.js'); await m.openVersionHistory('j1'); }],
