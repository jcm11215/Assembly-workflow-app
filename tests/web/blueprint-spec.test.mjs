...
import fs from 'fs';
const script = fs.readFileSync(new URL('../../web/scan/spec.js', import.meta.url), 'utf8')
  .replace(/^import.*$/gm, '')
  .replace(/^export /gm, '');
...
const kept = M.normalizeComponents(['Drive', 'Gear Motor', 'Reducer', 'Waste Pack Seal', 'Flange Bearing',
  'Hanger Bearing', 'Coupling Shaft', 'Tail Shaft', 'Drive Shaft', 'Shaft', 'Auger', 'UHMW Liner', 'Gasket', 'Coupling Bolts'].map(loc));
t('every listed part type is kept', kept.length === 14);
const dropped = M.normalizeComponents(['Drive End Plate', 'Drive Guard', 'Trough', 'Cap Screw', 'Shroud', 'Coupling', 'Sprocket',
  'Hex Bolt', 'Lock Washer'].map(loc));
