...
# --- CHECK 6 (global): openModal() calls missing a refresher ---
# A modal opened without a refresher sets modalRefresh=null, which makes
# refreshOpenModal() a silent no-op. Any handler in that modal that edits
# content then updates the database but never redraws the screen -- the
# exact failure mode of the BOM edit bug. Modals that are pure static
# forms are still safer with one, since a refresher is harmless when
# unused but its absence is invisible until something breaks.
for p, s in mods.items():
    # Strip comments first so a mention like "// not using openModal()"
    # isn't reported as a real call site.
    s_nc = re.sub(r'//[^\n]*', '', re.sub(r'/\*.*?\*/', '', s, flags=re.S))
    for m in re.finditer(r'openModal\(', s_nc):
        # crude arg scan: find the matching close paren for this call
        i, depth = m.end(), 1
        while i < len(s_nc) and depth:
            if s_nc[i] == '(': depth += 1
            elif s_nc[i] == ')': depth -= 1
            i += 1
        args = s_nc[m.end():i-1]
        # a refresher means a top-level comma outside any nesting
        d = 0; has_second = False
        for ch in args:
            if ch in '([{`': d += 1
            elif ch in ')]}`': d -= 1
            elif ch == ',' and d == 0: has_second = True; break
        if not has_second:
            line = s_nc[:m.start()].count(chr(10)) + 1
            add('HIGH','modal-no-refresher',p,f"openModal() at line {line} has no refresher -- refreshOpenModal() will silently no-op in this modal")
...
# Forms whose data-action is read by a delegated 'submit' listener
# (e.target.closest('[data-action="..."]')), not the click switch above.
handled |= set(re.findall(r'closest\([\'"]\[data-action="([a-z-]+)"', ev))
