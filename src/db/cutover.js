...
export function getMode(){
  ...
  // Relational-only is the default: this database was created fresh and
  // never had the legacy `app_data` blob table, so attempting a legacy
  // read just 404s on every boot before falling through. Devices that
  // explicitly set another mode still keep it.
  return ORDER.includes(m) ? m : MODE.RELATIONAL_ONLY;
}
