...
export function settingsModalHtml(){
  ...
  <div class="modal-sheet">
    ...
    <div id="geminiSection" ${provider!=='gemini'?'class="hidden"':''}>
      ...
      ${hasPersonalApiKey()
        ? `<div class="bp-file-chip">Using your own key: ${escapeHtml(masked)}</div>`
        : `<div class="bp-file-chip">Using the shop's shared key -- works with no setup. Add your own below only if this one stops working.</div>`}
      <form id="apiKeyForm">
        <div class="field">
          <label>API Key</label>
      ...
      ${hasPersonalApiKey() ? `<div class="fab-row"><button type="button" class="btn btn-outline btn-block" data-action="clear-api-key">Remove My Key -- Use Shop's Shared Key</button></div>` : ''}
    </div>
    ...
    <div id="openrouterSection" ${provider!=='openrouter'?'class="hidden"':''}>
      ...
      ${hasPersonalOpenRouterKey()
        ? `<div class="bp-file-chip">Using your own key: ${escapeHtml(orMasked)}</div>`
        : (orKey ? `<div class="bp-file-chip">Using the shop's shared key -- works with no setup. Add your own below only if this one stops working.</div>` : '')}
      <form id="openrouterKeyForm">
        ...
        <div class="field">
          ...
          <div class="bp-hint">Only free models that can actually read images are listed -- checked against OpenRouter's live model list, not guessed. Picking a paid model works too (via "Other"), but will use your OpenRouter balance per scan.</div>
        </div>
        <div class="fab-row">
          <button type="submit" class="btn btn-primary btn-block">Save</button>
        </div>
      </form>
      ${hasPersonalOpenRouterKey() ? `<div class="fab-row"><button type="button" class="btn btn-outline btn-block" data-action="clear-openrouter-key">Remove My Key -- Use Shop's Shared Key</button></div>` : ''}
    </div>
