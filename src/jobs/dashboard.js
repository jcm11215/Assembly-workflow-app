...
export function renderMetrics(){
  ...
  el.innerHTML = `
    <button class="metric-card m-blue${on('inprogress')}" data-action="goto-filter" data-filter="inprogress"><div class="num">${m.inProgress}</div><div class="lbl">In Progress</div></button>
    <button class="metric-card m-yellow${on('ready')}" data-action="goto-filter" data-filter="ready"><div class="num">${m.readyCount}</div><div class="lbl">Ready to Start</div></button>
    <button class="metric-card m-red${on('blocked')}" data-action="goto-filter" data-filter="blocked"><div class="num">${m.blockedCount}</div><div class="lbl">Blocked</div></button>
    <button class="metric-card m-amber${on('week')}" data-action="goto-filter" data-filter="week"><div class="num">${m.dueThisWeek}</div><div class="lbl">Due This Week</div></button>
    <button class="metric-card m-red${on('overdue')}" data-action="goto-filter" data-filter="overdue"><div class="num">${m.overdue}</div><div class="lbl">Overdue</div></button>
  `;
  ...
  const cardsHtml = jobs.length ? jobs.map(j=>{
    ...
    <div class="job-card due-${ds}" data-id="${j.id}" data-action="open-job-detail">
      <div class="job-card-top">
        ...
        ${j.blueprintThumbnail ? `<img src="data:image/jpeg;base64,${j.blueprintThumbnail}" class="job-thumb" alt="Blueprint for ${escapeHtml(j.jobNumber)}" data-action="open-blueprint-thumb" data-id="${j.id}">` : ''}
