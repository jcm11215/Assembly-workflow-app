...
export function renderMetrics(){
  ...
  // Highlight whichever metric matches the active filter, so it's clear
  // what the job list below is currently filtered by -- previously you
  // could tap a metric to filter and get no visual confirmation of it.
  const on = f => state.jobFilter === f ? ' active' : '';
  // A zero count isn't actionable, so it's dimmed -- keeps the eye on
  // the numbers that actually need something done about them.
  const z = n => n === 0 ? ' is-zero' : '';
  el.innerHTML = `
    <button class="metric-card m-blue${on('inprogress')}${z(m.inProgress)}" data-action="goto-filter" data-filter="inprogress"><div class="num">${m.inProgress}</div><div class="lbl">In Progress</div></button>
    <button class="metric-card m-yellow${on('ready')}${z(m.readyCount)}" data-action="goto-filter" data-filter="ready"><div class="num">${m.readyCount}</div><div class="lbl">Ready to Start</div></button>
    <button class="metric-card m-red${on('blocked')}${z(m.blockedCount)}" data-action="goto-filter" data-filter="blocked"><div class="num">${m.blockedCount}</div><div class="lbl">Blocked</div></button>
    <button class="metric-card m-amber${on('week')}${z(m.dueThisWeek)}" data-action="goto-filter" data-filter="week"><div class="num">${m.dueThisWeek}</div><div class="lbl">Due This Week</div></button>
    <button class="metric-card m-red${on('overdue')}${z(m.overdue)}" data-action="goto-filter" data-filter="overdue"><div class="num">${m.overdue}</div><div class="lbl">Overdue</div></button>
  `;
  ...
  const cardsHtml = jobs.length ? jobs.map(j=>{
    ...
    <div class="job-card due-${ds}" data-id="${j.id}" data-action="open-job-detail">
      <div class="job-card-main">
        ${j.blueprintThumbnail
          ? `<button type="button" class="job-thumb-btn" data-action="open-blueprint-thumb" data-id="${j.id}" title="Open the full blueprint">
               <img src="data:image/jpeg;base64,${j.blueprintThumbnail}" class="job-thumb" alt="Blueprint for ${escapeHtml(j.jobNumber)}">
               <span class="job-thumb-hint">View</span>
             </button>`
          : `<div class="job-thumb-empty" title="No blueprint scanned yet">&#128208;</div>`}
        <div class="job-card-body">
          <div class="job-card-top">
            ...
            <span class="badge badge-${(j.priority||'Medium').toLowerCase()}">${escapeHtml(j.priority||'Medium')}</span>
          </div>
          ...
          <div class="job-meta-row">
            ...
            <span>Due: <b>${fmtDate(j.dueDate)}</b> <span class="due-tag due-${ds}">${dueText}</span></span>
          ...
          <div class="gauge">
            ...
            <div class="gauge-label">${j.percentComplete}% complete</div>
          </div>` : ''}
        </div>
      </div>
