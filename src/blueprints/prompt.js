...
COMPONENTS LIST -- what belongs in it:
- Always include the motor and the gearbox/reducer themselves as their own line items (installation_location "drive_end") when visible, not just as the signal used to establish orientation in Pass 1 above.
- Do NOT include trough sections or discharge spouts/chutes as line items in "components" -- these are structural, not hardware, and are not wanted in this list. (The "trough" object above, with its dimensions, still applies and should still be filled in -- this exclusion is only about the components array.)
- Everything else the drawing shows as installable hardware (shafts, bearings, couplings, sprockets, keys, seals, plates, guards, hangers, fasteners called out by size) belongs in the list as normal.
