...
import { BOM_BUCKET_META, BOM_BUCKET_ORDER, BOM_SUBGROUP_LABEL, BUCKET_TO_STAGE, bomBucketFor, isEndHardware } from '../models/stageMeta.js';
...
function componentRowHtml(c, bucket, idx, total, editing){
  ...
  // The arrows only reorder WITHIN a category. Moving a part the scanner
  // filed wrong into the right category is the more common fix, and it
  // needs its own control -- buckets are often a single item, which
  // leaves both arrows correctly disabled and nothing else to do.
  const categorySelect = editing ? `
    <select class="bom-category-select" data-action="bom-set-category" data-component-id="${c.id}">
...
export function bomListHtml(job){
  ...
  const sections = bucketsToShow.map(b=>{
    ...
    const subLabel = BOM_SUBGROUP_LABEL[b];

    let body;
    if(subLabel){
      // Drive End / Tail End: the end assembly itself, then its plate,
      // bearing and seal hardware nested underneath as a sub-group.
      const main = items.filter(c=>!isEndHardware(c));
      const hw   = items.filter(c=>isEndHardware(c));
      body =
        `<div class="bom-list">${main.map((c,i)=>componentRowHtml(c, b, i, main.length, editing)).join('')
          || (editing?'<div class="bp-hint" style="margin:4px 0;">Nothing here yet.</div>':'')}</div>` +
        (hw.length || editing
          ? `<div class="bom-subgroup-head">${escapeHtml(subLabel)} <span class="checklist-badge">${hw.length}</span></div>
             <div class="bom-list bom-sublist">${hw.map((c,i)=>componentRowHtml(c, b, i, hw.length, editing)).join('')
               || (editing?'<div class="bp-hint" style="margin:4px 0;">Nothing here yet.</div>':'')}</div>`
          : '');
    } else {
      body = `<div class="bom-list">${items.map((c,i)=>componentRowHtml(c, b, i, items.length, editing)).join('')
        || (editing?'<div class="bp-hint" style="margin:4px 0;">Nothing here yet.</div>':'')}</div>`;
    }
