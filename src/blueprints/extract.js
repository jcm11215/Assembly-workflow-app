...
import { fileToBase64Raw, fileToImageBase64Resized, pdfFileToImages } from './pdf.js';
...
export async function extractComponents(jobId){
  ...
  try{
    const { contentBlocks, originalFile } = await contentBlocksFor(file);
    ...
    const saved = await blueprintsRepo.saveExtraction(job.id, { components, originalFile });
...
export async function extractNewJobFromBlueprint(){
  ...
  try{
    ...
    const prefill = {
      ...
      _pendingBlueprint: { components, originalFile }
    };
