...
// Discharge parts vary in naming across drawings ("discharge spout",
// "discharge chute", "twin discharge") -- matched by keyword since
// there's no dedicated installation_location value for them.
const DISCHARGE_RE = /\bdischarge\b/i;
...
export function normalizeComponents(parsed){
  ...
  return raw
    // Troughs and discharge parts are excluded from the components list
    // by design (prompt.js instructs the AI not to include them), but
    // filtered here too rather than trusting that instruction alone --
    // a prompt change is one bad extraction away from silently drifting,
    // this filter cannot.
    .filter(c => c && c.installation_location !== 'trough' && !DISCHARGE_RE.test(c.item || ''))
    .map(c=>{
...
export function validateComponents(spec, components){
  ...
  // Cross-check every component's installation_location against the
  // structured drive.location field the same extraction reported. These
  // two come from different parts of the prompt (per-component tagging
  // vs. the overall drive-end determination), so a contradiction between
  // them is a strong, specific signal of exactly the drive/tail mixup
  // this check exists to catch -- not a generic "something looks off."
  const driveLoc = spec.drive && spec.drive.location; // 'head' | 'tail' | null
  if(driveLoc === 'head' || driveLoc === 'tail'){
    const oppositeLocation = driveLoc === 'head' ? 'tail_end' : 'drive_end';
    const expectedLocation = driveLoc === 'head' ? 'drive_end' : 'tail_end';
    list.forEach(c=>{
      // Only components the model tagged with real confidence -- a low-
      // confidence "unknown" isn't a contradiction, it's already honest.
      if(c.installation_location === oppositeLocation && c.confidence >= 0.6){
        add('conflict',
          `"${c.item}" is tagged ${oppositeLocation.replace('_',' ')} but the extraction's own drive.location field says the drive is at the ${driveLoc} end -- these two parts of the same extraction disagree about which end is which. Expected this component to be ${expectedLocation.replace('_',' ')} or hanger.`);
      }
    });
  }
